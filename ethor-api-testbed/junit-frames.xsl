...
<xsl:param name="TITLE">ethor RestAPI test report</xsl:param>
...
<!--     <redirect:write file="{$output.dir}/overview-frame.html">
    ...
    <redirect:write file="{$output.dir}/modules.html">
        <xsl:apply-templates select="." mode="all.modules"/>
    </redirect:write>
...
<html>
    ...
    <frameset cols="20%,80%">
		<frame src="modules.html" name="packageListFrame"/>
        <frame src="overview-summary.html" name="classFrame"/>
        <noframes>
            <h2>Frame Alert</h2>
            <p>
...
<xsl:template match="testsuite" mode="class.details">
    ...
    <html>
        <head>
          <title>Unit Test Results: <xsl:value-of select="@name"/></title>
            <xsl:call-template name="create.stylesheet.link">
...
<xsl:template match="testsuites" mode="all.modules">
    <html>
        <head>
            <title>All Unit Test Packages</title>
            <xsl:call-template name="create.stylesheet.link">
        ...
        <body>
            <h2><a href="overview-summary.html" target="classFrame">Home</a></h2>
            <h2>ethor RestAPI modules</h2>
            <table width="100%">
                <xsl:apply-templates select="testsuite[not(./@package = preceding-sibling::testsuite/@package)]" mode="all.modules">
                    <xsl:sort select="@package"/>
                </xsl:apply-templates>
            </table>
        </body>
...
<xsl:template match="testsuite" mode="all.modules">
    <tr>
        <td nowrap="nowrap">
            <a href="./{translate(@package,'.','/')}/package-summary.html" target="classFrame">
                 <xsl:if test="@package = 'com.ethor.testbed.api.test.chain'">Chains</xsl:if>
                 <xsl:if test="@package = 'com.ethor.testbed.api.test.category'">Category</xsl:if>
                 <xsl:if test="@package = 'com.ethor.testbed.api.test.restaurant'">Restaurants</xsl:if>
                 <xsl:if test="@package = 'com.ethor.testbed.api.test.menuitem'">Menu items</xsl:if>
                 <xsl:if test="@package = 'com.ethor.testbed.api.test.special'">Specials</xsl:if>
                 <xsl:if test="@package = 'com.ethor.testbed.api.test.order'">Orders</xsl:if>
                <xsl:if test="@package = ''">&lt;none&gt;</xsl:if>
            </a>
        </td>
    </tr>
